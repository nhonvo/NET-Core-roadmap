...
using StudentManageList.API.AppResult;
using StudentManageList.Core.Models.StudentViewModel;

namespace StudentManageList.API.Controllers
{
    [ApiController]
    ...
    public class StudentController : ControllerBase
    {
        ...
        {
            var result = await DataSeeder.GetByIdAsync(id);
            if (result == null)
        ...
        {
            var result = await DataSeeder.GetAllAsync();
            if (result == null)
        ...
        public async Task<ApiResult<Student>> CreateAsync([FromBody] StudentDTO studentDTO)
        {
            ...
            try
            {
                ...
                await DataSeeder.AddAsync(student);
        ...
        public async Task<ApiResult<Student>> UpdateAsync(int id, [FromBody] StudentDTO studentDTO)
        {
            ...
            try
            {
                Student currentStudent = await DataSeeder.GetByIdAsync(id);
                ...
                DataSeeder.Update(currentStudent);
        ...
        public async Task<ApiResult<Student>> PatchAsync(int id, [FromBody] JsonPatchDocument<StudentDTO> patchDocument)
        {
            ...
            try
            {
                var currentStudent = await DataSeeder.GetByIdAsync(id);
                if (currentStudent == null)
                {
                    return new ApiErrorResult<Student>(message: "Student not found");
                }
        ...
        public async Task<ApiResult<bool>> DeleteAsync(int id)
        {
            try
            {
                var student = await DataSeeder.GetByIdAsync(id);
                if (student == null)
                {
                    return new ApiErrorResult<bool>(message: "Student not found");
                }

                DataSeeder.Delete(id);
